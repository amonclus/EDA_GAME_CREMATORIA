...
struct PLAYER_NAME : public Player
{
     ...
     void check_enemy(Pos p, int id, bool &enemy)
     {
          for (int i = p.i-3; i <= p.i+3; ++i){
               for(int j = p.j-3; j<= p.j+3; ++j){
                    Pos new_p = Pos(i,j,0);
                    if(cell(new_p).id != -1 and unit(cell(new_p).id).player != me()){        //this means there is an enemy here
                         dijstra(p, new_p, id, n_cells);
                         break;
                    }
               }
          }
     ...
     void init_distance(map<Pos, int> &distance, int n){
          for(int i = 0; i < 40; ++i){
               for(int j = 0; j < 80; ++j){
                    Pos p = Pos(i,j,0);
                    if(cell(p).type != Rock){
     ...
     void dijstra(Pos pos, Pos dest, int id, int n){
          map<Pos, int>distance;        //hay que inicializar esto de alguna forma
          init_distance(distance, n);
          map<Pos, bool> vis;
          map<Pos, Pos>prev;
          distance[pos] = 0;
          stack<Pos> Q;
          ...
          while(! Q.empty()){
               ...
               if(not v){
                    vis[u] = true;
                    for(long unsigned int i = 0; i  < adj[pos].size(); ++i){             //recorremos todos los vecinos
                         Pos v = adj[pos][i];          //primer vecino
          ...
          Pos pre = prev[dest];         //previo al del final
          while(distance[pre] != 1){
               pre = prev[pre];
          }
          find_pos(id, pos, pre);
     }
               ...
               if(not hound){
                    ...
                    {    
                         ...
                         for (int p = x; p < x + 8; ++p)
                         {
                              ...
                              Cell c = cell(new_p);
                              if (c.type != Rock and unit(cell(new_p).id).player != me())
                              {
                                   command(id, Dir(p_n));
                                   break;
                              }
               ...
               for(int i = 0; i < 40; ++i){
                    for(int j = 0; j < 80; ++j){
                         ...
                         if(cell(p).type != Rock){
                              ...
                              for(int k = 0; k < 8; ++k){
                                   Pos n_pos = p+= Dir(k);
                                   if(n_pos.j <80 and n_pos.i < 40)
                                        if(cell(n_pos).type != Rock) adj[p].push_back(n_pos);
                              }
                         }
                    }
               }
